...
export function GroceryList({ mealPlan, pantryItems = [] }: GroceryListProps) {
  ...
  const [pendingInstacartUrl, setPendingInstacartUrl] = useState<string | null>(null)
  ...
  const handleOrderInstacart = async () => {
    ...
    try {
      ...
      const oauthToken = localStorage.getItem("instacart_oauth_token")

      if (oauthToken) {
        // Use Connect API for authenticated users
        const tokenData = JSON.parse(oauthToken)

        // Check if token is still valid
        if (Date.now() < tokenData.expiresAt) {
          // TODO: Implement store selection and order placement flow
          toast({
            title: "Connect API Integration",
            description: "Store selection and order placement coming soon!",
          })
          return
        } else {
          // Token expired, remove it
          localStorage.removeItem("instacart_oauth_token")
          toast({
            title: "Session Expired",
            description: "Please reconnect your Instacart account in Settings.",
            variant: "destructive",
          })
          return
        }
      ...
      // Fall back to IDP API for non-authenticated users
      ...
      if (data.url) {
        const skipConfirmation = localStorage.getItem("instacart_skip_confirmation") === "true"
        ...
        if (skipConfirmation) {
          ...
          setPendingInstacartUrl(data.url)
  ...
  const handleConfirmInstacart = () => {
    if (pendingInstacartUrl) {
      ...
      openInstacartURL(pendingInstacartUrl)
  ...
  return (
    <>
      <InstacartConfirmationDialog
        open={showConfirmationDialog}
        onOpenChange={setShowConfirmationDialog}
        onConfirm={handleConfirmInstacart}
        itemCount={groceryList.flatMap((group) => group.items.filter((item) => !checkedItems.has(item.id))).length}
      />
