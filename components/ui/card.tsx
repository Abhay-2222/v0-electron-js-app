...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn("bg-card text-card-foreground flex flex-col gap-4 rounded-2xl py-5 shadow-warm-sm border border-border/60", className)}
      {...props}
    />
  )
}
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return <div data-slot="card-title" className={cn("leading-snug font-normal text-[15px]", className)} {...props} />
}
